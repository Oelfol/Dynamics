        ...
        for pair in chosen_pairs:
            for t in range(total_t):
                for j in range(2):
                    ...
                    choose_control_gate(alpha, qc_id, 0, pair[0] + 1)
                    real_or_imag_measurement(qc_id, j)
                    measurement_id = self.run_circuit(1, qc, False) / pseudo_constant
...
def time_ev_op_decomp(qc, a, b, j, t):  # modify these to do the same?
...
def four_site_ibmq(total_time, dt, j):  # modify these to do the same?
    # For Francis Fig 5a
...
# spins9.all_site_magnetization(spins9.second_order_trotter, total_time=305, dt=.01, initialstate=0, hadamard=True)
...
# QUANTUM 1ST ORDER
# spins6 =QuantumSim(j=1, bg=20, n=3, a = 1, j2=1, choice=False, open_chain=True, paper='tachinno', error=0.05)
# i = int('100', 2)                                 # initial state
# c = [int(x, 2) for x in ['100', '010', '111']]    # chosen states
# spins6.occupation_probabilities(spins6.first_order_trotter, total_time=305, dt=.01, initialstate=i, chosen_states=c)
