...
# Plotting helpers for spin chains code examining readout error mitigation
# NEED to emphasisze = plotting helpers for error mitigated circuits only
...
def two_point_correlations_plotter_file(filename):
    vars = sch.read_var_file(filename + "_vars.csv")[0]
    n, j, total_t, dt = int(vars[0]), vars[1], int(vars[2]), vars[3]
    pairs = sch.read_var_file(filename + "_pairs.csv")[0]
    data_real_cl = sch.read_numpy_array(filename + "_cl_real.txt")
    data_imag_cl = sch.read_numpy_array(filename + "_cl_imag.txt")
    data_real_rm = sch.read_numpy_array(filename + "_q_real_RM.txt")
    data_imag_rm = sch.read_numpy_array(filename + "_q_image_RM.txt")
    data_real = sch.read_numpy_array(filename + "_q_real_nRM.txt")
    data_imag = sch.read_numpy_array(filename + "_q_imag_nRM.txt")
...
def total_magnetization_plotter_file(filename):
    vars = sch.read_var_file(filename + "_vars.csv")[0]
    n, j, total_t, dt = int(vars[0]), vars[1], int(vars[2]), vars[3]
    data_cl = sch.read_numpy_array(filename + "_cl.txt")
    data_rm = sch.read_numpy_array(filename + "_q_RM.txt")
    print(data_rm)
    data = sch.read_numpy_array(filename + "_q_nRM.txt")
...
def all_site_magnetization_plotter_file(filename):
    ...
    n, j, total_t, dt = int(vars[0]), vars[1], int(vars[2]), vars[3]
    data_cl = sch.read_numpy_array(filename + "_cl.txt")
    data_two_noRM = sch.read_numpy_array(filename + "_q_nRM.txt")
    data_two_RM = sch.read_numpy_array(filename + "_q_RM.txt")
...
all_site_magnetization_plotter_file('joel_Oct25')
...
def total_magnetization_plotter(j, total_t, dt, data, data_gates, data_cl):
    ...
    # Gates plots still have an issue here go to notebook 2 TPDP
...
def all_site_magnetization_plotter_b(n, j, dt, total_t, data_one_noise, data_cl):
    ...
    for site in range(n):
        ...
        sitedex += 1
    ph.plot_dataset_byrows(ax, 'Sites', 'Magnetization', r'$\it{Jt}$')
    fig.suptitle('Magnetization per Site', fontsize=16)
    plt.show()
